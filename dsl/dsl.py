...
class Transformer:
    ...
    def neighborhood(self, pixel: Pixel) -> Object:     #We want to determine the neighbourhood pixels of a pixel. 
    ...
    def pixel_out_with_uncovered_neighbors_with_diagonal(self, object: Object) -> Tuple[Object, Object]:
        ...
        for pixel_1 in object: 
            neighborhood_pixel_1 = self.neighborhood_with_diagonals(pixel_1) #@Lorenz this requires an object, you are giving it a pixel. I changed the function.
            local_uncovered = set(neighborhood_pixel_1)  # Start with all neighbors.
    ...
    def isolate(self, object: Object) -> (Object): #isolate an object
        ...
        print('these would be the new values for x_max and y_max:', self.x_max(object), self.y_max(object))
        self.grid_width = self.x_max(object) - self.x_min(object) +1
        self.grid_height = self.y_max(object) - self.y_min(object) +1
        return outcome
    ...
    def project_triplicate(self, object: Object) -> Object: #triplication of an object
        ...
        self.grid_width = self.grid_width *3
        return outcome
        ...
        for (pixel) in object: 
            for x_value in range(0,5):
                for y_value in range(0,5):
                    ...
                    outcome.add(newpixel)
        self.grid_height = self.grid_height *5
        self.grid_width = self.grid_width *5
        return outcome
    ...
    def project_half(self, object: Object) -> Object: #project on grid of half size
        ...
        self.grid_height = int(self.grid_height /2)
        self.grid_width = int(self.grid_width /2)
        return outcome 
    ...
    def project_third(self, object: Object) -> Object: #project on grid of third size
        ...
        self.grid_height = int(self.grid_height /3)
        self.grid_width = int(self.grid_width /3)
        return outcome 
    ...
    def project_fifth(self, object: Object) -> Object: #project on grid of fifth size
        ...
        for value_1 in range(5): #to shrink x-value
            for value_2 in range(5): # to shrink y-value
                ...
                if self.color_max(subgrid) is not None:    
                    outcome.add((value_1, value_2, self.color_max(subgrid))) #add new pixel to final object
                #@Lorenz: If you want to projec the whole grid to a lower dimension, we need to leavet eh second part out! I then added in resizing the grid. We should take about which effect we actually want. I also changed it for the other project smaller functions
                # else:
                #     outcome.add((value_1, value_2, 0))
        print(outcome)
        
        self.grid_height = int(self.grid_height /5)
        self.grid_width = int(self.grid_width /5)
        return outcome 
    

    def add_star_around_object(self, object: Object) -> Object: # add star-like pixels
        outcome = object
        outside_pixels, uncovered_neighbors = self.pixel_out_with_uncovered_neighbors(object) # then we add the surronding objects
    ...
    def add_corners_around_object(self, object: Object) -> Object: # add diagonal corners
        outcome = object
        outside_pixels, uncovered_neighbors = self.pixel_out_with_uncovered_neighbors_only_diagonal_neighborhood(object) # then we add the surronding corners
        for pixel in uncovered_neighbors:
            outcome.add((pixel[0],pixel[1], self.color)) #they get the assigned color
    ...
    def add_border_around_object(self, object: Object) -> Object: # add boundary
        outcome = object
        outside_pixels, uncovered_neighbors = self.pixel_out_with_uncovered_neighbors_with_diagonal(object) # then we add the surronding corners
        for pixel in uncovered_neighbors:
            outcome.add((pixel[0],pixel[1], self.color)) #they get the assigned color
    ...
    def grid_add_down(self, object: Object) -> Object: #add one more gridline at the bottom
        ...
        for x_value in range(self.grid_width): 
            ...
            outcome.add(newpixel)
        self.grid_height = self.grid_height +1
        return outcome
    ...
    def grid_add_right(self, object: Object) -> Object: #add one more gridline right
        ...
        for y_value in range(self.grid_height):
            ...
            outcome.add(newpixel)
        self.grid_width = self.grid_width +1
        return outcome
